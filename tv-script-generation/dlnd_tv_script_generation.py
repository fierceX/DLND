...
def create_lookup_tables(text):
    ...
    for w in text:
        if w not in vocab_to_int:
            vocab_to_int[w] = len(vocab_to_int)
            int_vocab[len(vocab_to_int)-1] = w
...
def get_inputs():
    ...
    """
    # TODO: Implement Function
    return tf.placeholder(tf.int32,shape=(None, None), name='input'), tf.placeholder(tf.int32,shape=(None, None), name='targets'), tf.placeholder(tf.float32,shape=(None), name='LearningRate')
...
def get_init_cell(batch_size, rnn_size):
    ...
    State = stacked_rnn_cell.zero_state(batch_size=batch_size,dtype=tf.float32)
    InitialState = tf.identity(State,name='initial_state')
    
    return stacked_rnn_cell, InitialState
...
def get_embed(input_data, vocab_size, embed_dim):
    ...
    """
    # TODO: Implement Function
    word_embed = tf.get_variable("word_embeddings",[vocab_size, embed_dim],dtype=tf.float32)
    embed = tf.nn.embedding_lookup(word_embed,ids=input_data)
    return embed
